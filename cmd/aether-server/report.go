...
// It sits under the 5 s timeout the harness gives the hook, because the
// hook runs on the agent's own turn boundaries: being late is worse than
// being wrong.
const reportBudget = 4 * time.Second

// maxHookPayload bounds the hook body read from stdin. A hook payload is
// a few hundred bytes of metadata; anything past this is not one.
...
// the transcript.
//
// Adding a harness is one case here plus one mapping function in
...
func report(args []string) {
	...
	case "claude":
		...
		if err != nil {
			warn("report claude: read the hook payload: %v", err)
			return
		}
		rep, mapped = agentstatus.FromClaudeHook(payload)
	default:
		warn("report: unknown harness %q", args[0])
		return
	}
	...
	defer cancel()
	err := mcpbridge.Call(ctx, mcpbridge.SocketPath, protocol.MethodRunReport, protocol.RunReportParams{
		State:  string(rep.State),
		Reason: rep.Reason,
	}, nil)
	if err != nil {
		warn("report %s: %v", args[0], err)
	}
}
