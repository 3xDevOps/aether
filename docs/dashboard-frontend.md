...
editor renders complete UTF-8 text without NUL bytes; binary responses remain
read-only.
...
- **Colour is the whole of the highlighting.** `parse.ts` splits the unified
  ...
  sends the complete run diff, and the parser remains tolerant of incomplete
  input from a failed transport.
