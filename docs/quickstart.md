...
dashboard wizard's Repository step does the same. Recording happens only when
...
also continues profile discovery/push, reconnect, and live-overlay features.
It does not fetch a mirror source or forward a checkout's `origin` into the
