...
| `TestIntegrationAgentStatusReporterInContainer` (`agentstatus_integration_test.go`) | The status reporter inside a real container on the shipped `claude` profile: the settings document written at `0444` into the run's coordination directory, `--settings` pointing the harness at it, the staged binary running `aether-server report claude` against the run's own socket, and the run parking at needs-attention with `waiting for your input` seconds after the agent's turn ends - not after the stall threshold - then returning to running with `agent resumed` on the agent's next turn |
...
| Agent waiting for a human | Both status reporter E2Es in a real container (`claude` pointed at its hooks by flag, `opencode` at its plugin by environment); the park/un-park matrix in `internal/scheduler` unit tests (a waiting report replaces a stall reason, a repaint does not un-park a reporting harness, silence still parks a working one, and a waiting report survives a server restart); the event mapping tables in `internal/agentstatus`, where the opencode plugin is also driven through a turn under `node` (skipped where there is none); `run.report` dispatch and its refusals in `internal/coord`; the `report` subcommand against a real socket in `cmd/aether-server` |
