...
`terminal-images` and `terminal-tools` need a reachable Docker daemon and skip without one. That
...
| `run-views.mobile` | On a phone, steering a real run from the one Actions menu the run header keeps, and then reading its diff: the menu items are finger-sized, protecting the run shows on the header, and the patch reads without scrolling the page sideways |
...
need Docker, because they open a real run, and skip without it. Run them alone against the binaries
`make build` produced:
