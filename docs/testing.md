...
| `TestIntegrationNativeRunActivity` (`run_activity_integration_test.go`) | OMP/Pi native terminal titles drive working -> idle/needs-attention -> working -> blocked/needs-attention -> working -> completed. Steering remains available while waiting; unrelated output cannot clear attention. Checks the server event bus and SSH `run.get`/`run.list`; Docker executes the scripted agent, or the in-process runtime emits the same title bytes when Docker is unavailable |
