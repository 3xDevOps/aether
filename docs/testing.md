...
`shell-drawer.mobile.spec.ts`, `terminal-images` and `terminal-tools` need a
...
The phone specs need git; `shell-drawer.mobile.spec.ts` and
`run-views.mobile.spec.ts` also need Docker, because they open a real run, and
skip without it. Run them alone against the binaries `make build` produced:
...
| Agent waiting for a human | The status reporter E2E in a real container; the park/un-park matrix in `internal/scheduler` unit tests (a waiting report replaces a stall reason, a repaint does not un-park a reporting harness, silence still parks a working one, and a waiting report survives a server restart); the hook mapping table in `internal/agentstatus`; `run.report` dispatch and its refusals in `internal/coord`; the `report` subcommand against a real socket in `cmd/aether-server` |
