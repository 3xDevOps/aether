...
func TestBridgeAgainstRealCoordination(t *testing.T) {
	...
	// retires that batch and A's inbox is empty. The token is promoted on
	// the bridge's write path, after callTool already decoded the result,
	// so a read can race ahead of it and be handed the same batch again.
	// That redelivery is by design; read until the batch is retired.
	readUntilEmpty(t, agentA, &inbox)
...
func TestRealBatchRedeliversWhenTheResponseIsLost(t *testing.T) {
	...
	// a following one retires it, once the response that promotes its
	// token has been written.
	readUntilEmpty(t, revived, &inbox)
	if len(inbox.Messages) != 0 {
