...
func (s *Service) integratorLaunchFailure(ctx context.Context, m *domain.Mission, err error) error {
	...
	switch {
	case getErr == nil:
		return fmt.Errorf("mission %s exists but its integrator run %s failed to start; replace the integrator from the Missions page or inspect with aether swarm show %s: %w", m.ID, m.CurrentIntegratorRunID, m.ID, err)
