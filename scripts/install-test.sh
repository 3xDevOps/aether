...
# Stands in for the real binary: `config path` reports where the config
# lives, and a completed setup leaves that file behind. SETUP_FAILS is a
# setup that errors; SETUP_DECLINES is the operator answering no to the
# summary, which the real command reports by exiting 0 having written
# nothing.
...
for asset in aether-linux-amd64 aether-server-linux-amd64; do
	src="$fixtures/aether"
...
export CMD_LOG="$log" FIXTURES="$fixtures" FAKE_CONFIG="$tmp/server.conf"
...
reset() {
	...
	: >"$log"
	: >"$out"
	rm -f "$bindir/aether" "$bindir/aether-server" "$tmp/server.conf"
}
...
# --- no terminal and no role: ask nothing, run nothing, do not block -----

reset "no tty, no role"
detach=""
...
if [ -n "$detach" ]; then
	...
	echo "install-test: no setsid and a terminal is attached; skipping the no-tty case" >&2
fi
