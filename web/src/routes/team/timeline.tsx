...
function Select({
  ...
  return (
    <Label className="flex min-w-0 flex-col items-stretch gap-1 text-[13px] font-medium text-foreground">
      ...
      <select
        ...
        className={cn(
          focusRing,
          'h-9 min-w-0 w-full truncate rounded-sm border border-input bg-background px-2.5 text-sm text-foreground',
        )}
      >
        {options.map(([id, name]) => (
