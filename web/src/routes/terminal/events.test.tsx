...
import { alice, fakeApi, run, session } from '@/test/fixtures'
...
describe('run events tab', () => {
  it('pins the feed to the run and lists its events newest first', async () => {
    ...
    for (const [q] of vi.mocked(client.sessionTimeline).mock.calls) {
      expect(q.session_id).toBe(session.id)
  ...
  it('hands the shared filters back on unmount', async () => {
    seed()
    const chosen = { ...emptyFilters, sessionID: 'ses_other', type: 'run.status' }
